...
#ifdef SWIG
#define _MAKE_POSITION_TEMPLATE(_n,_t,_s) typedef KERNEL::Point<_t,_s> _n;
#else
#define _MAKE_POSITION_TEMPLATE(_n,_t,_s) typedef KERNEL::Point<_t,_s> _n;   \
namespace _n ## s                                     \
...
#pragma warning( pop )
        ...
        bool operator == (const Point<_T,__Scale> &pos) const
        {
            ...
            // it might cause problem here, since the comparison might not be suitable for every type.
            if (this->x == pos.x &&
                this->x == pos.y) {
                return true;
            }else{
        ...
        bool operator  < (const Point<_T,__Scale> &position) const
        {
            ...
            //error maybe happen if understanding about std::tie
