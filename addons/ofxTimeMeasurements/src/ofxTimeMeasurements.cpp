...
float ofxTimeMeasurements::pauseMeasuring(string ID){
	...
	if ( it == times.end() ){	//not found!
		...
		ofLog( OF_LOG_WARNING, "ID (%s)not found at pauseMeasuring(). Make sure you called startMeasuring with that ID first.", ID.c_str());
		...
		if ( times[ID].measuring ){
			...
			ofLog( OF_LOG_WARNING, "Can't pauseMeasuring(%s). Make sure you called startMeasuring with that ID first.", ID.c_str());
		}		
	}
	return ret;
