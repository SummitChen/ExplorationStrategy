...
class pos2 {
	public:
		pos2(short x_, short y_){ x = x_; y = y_;}
		pos2(){ x = y = 0;}
		short x, y;
};
...
class ofxMapRouting{
	...
	public:
		...
		bool calcRoute( int originX, int originY, int destX , int destY, std::vector<pos2> &solution, int maxIterations = 500);
	
		//vector<pos2> getSolution(){ return solution;} //solution is returned in reverse order ( solution[0] == destination)
	...
	private:
		...
		bool calcRoute( Node* origin, Node* destination, std::vector<pos2> &solution, int maxIterations);
		...
		void reconstructPath( Node* destination, std::vector<pos2> &solution );
