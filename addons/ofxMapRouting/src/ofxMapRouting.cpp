...
bool ofxMapRouting::calcRoute( int originX, int originY, int destX , int destY, std::vector<pos2> &solution, int maxIterations){
...
bool ofxMapRouting::calcRoute( Node* origin, Node* destination, std::vector<pos2> &solution, int maxIterations){
    ...
    printf("Node insert - (%d, %d)", origin->x, origin->y);
    if ( origin->parent == NULL) {
        printf("   parent is NULL");
    }else{
        printf("   parent - (%d, %d)", origin->parent->x, origin->parent->y);
    }
	...
	while ( !open.empty() ){
		...
		for ( int i = 0; i < neighborNodes.size(); i++ ){
			...
			if ( 0 == closed.count(neighbor) ){ // neighbor is not in the closed list
				...
				float updatedCost = current->g + roadCostGain * gameMap->getTileSafe(neighbor->x, neighbor->y) + 0.00f + perStepExtraCost;
...
#ifdef PLANNING_DEBUG
                        printf("Node insert - (%d, %d)", neighbor->x, neighbor->y);
                        if ( neighbor->parent == NULL) {
                            printf("   parent is NULL");
                        }else{
                            printf("   parent - (%d, %d)", neighbor->parent->x, neighbor->parent->y);
                        }
...
void ofxMapRouting::reconstructPath( Node* destination, std::vector<pos2> &solution ){
	...
	while (destination != NULL){
        ...
        if ( destination->x == 0 &&
            destination->y == 0) {
            printf("current position (0, 0)");
            if ( destination->parent == NULL) {
                printf(" its parent is NULL \n");
            }else{
                printf(" its parent is (%d, %d)\n", destination->x, destination->y);
            }
        }
        
        printf("destination (%d, %d), transfer (%d, %d)", destination->x, destination->y, p.x, p.y);
        
        printf("into solution end (%d, %d) - (%d, %d), front (%d, %d), first (%d, %d)\n", solution.end()->x, solution.end()->y, solution.at(solution.size()-1).x, solution.at(solution.size()-1).y, solution.begin()->x, solution.begin()->y, solution.at(0).x, solution.at(0).y);
...
void ofxMapRouting::addNode(int xx, int yy, vector<Node*> &l){
	...
	if ( gameMap->getTileSafe(xx, yy) == WALKABLE ){
